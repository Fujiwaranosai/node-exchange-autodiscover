 ...
 * @returns {Promise}
 */
function tryEndpoint(url, username, password, requestBody, proxy) {
  if (!proxy) {
	return fetch(url, {
		method: 'POST',
		headers: {
 ...
 * @returns {Promise}
 */
function autodiscoverDomains(domains, emailAddress, password, username, settings, proxy) {
  const promises = [];
  ...
  domains.forEach(domain => {
    promises.push(tryEndpoint(`https://${domain}/autodiscover/autodiscover.svc`, username, password, requestBody, proxy));
    promises.push(tryEndpoint(`https://autodiscover.${domain}/autodiscover/autodiscover.svc`, username, password, requestBody, proxy));
    promises.push(fetch(`http://autodiscover.${domain}/autodiscover/autodiscover.svc`, {agent, redirect: 'manual'}).then(response => {
      // Just take redirects into consideration.
      if (response.status !== 302) {
        throw new Error();
      }
      return tryEndpoint(response.headers.get('location'), username, password, requestBody, proxy);
    }));
  });
  ...
  return Promise
    ...
    .then(result => {
      ...
      // TODO: we should use abort signal to cancel a fetch process
...
module.exports = function (params, cb) {
  ...
  return promise.then(otherDomains => {
    ...
    return autodiscoverDomains(domains, emailAddress, password, username, requestedSettings, proxy);
  }).then(settings => {
